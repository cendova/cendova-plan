/**
 * Gemeinsame Geometrie-Primitiven aller Planungsmodule (Hüfte, Knie,
 * Schulter). Reine Mathematik in Weltkoordinaten — KEINE anatomische
 * Semantik, keine Modul-Abhängigkeiten.
 *
 * Herkunft: Diese Funktionen standen bis dahin in `lib/hip/geometry.ts`;
 * `lib/knee/geometry.ts` importierte sie von dort und leitete sie weiter
 * (bewusst als Fassade angelegt, „damit der Knie-Code konsistent bleibt,
 * wenn die Hüft-Geometrie später woandershin wandert"). Genau das ist
 * hier passiert — verschoben, nicht verändert.
 *
 * Muster für alle Module: `lib/<modul>/geometry.ts` re-exportiert diese
 * Primitiven und ergänzt die modul-EIGENEN Helfer (Hüfte: Becken-
 * Referenzlinie; Knie: Gelenklinien-Winkel; Schulter: Skapula-Achsen).
 * So gibt es die Basis-Mathematik genau EINMAL — ohne sie wären es bei
 * drei Modulen drei Kopien, die auseinanderdriften (der Grund, aus dem
 * `refLineFrame` seinerzeit eingeführt wurde).
 */
 ...
 * Für Gelenkkopf-Zentren aus drei Konturpunkten (Hüftkopf, Humeruskopf).
 *
